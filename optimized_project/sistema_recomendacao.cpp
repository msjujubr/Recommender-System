...
float similaridade_cosseno(const std::unordered_map<int, float>& userA, const std::unordered_map<int, float>& userB) {
    ...
    // Choose smaller map for iteration to optimize performance
    const auto& smaller = (userA.size() < userB.size()) ? userA : userB;
    const auto& larger = (userA.size() < userB.size()) ? userB : userA;

    // Itera sobre os filmes avaliados pelo usuário com menos avaliações
    for (auto it = smaller.begin(); it != smaller.end(); ++it) {
        auto other_rating = larger.find(it->first);
        if (other_rating != larger.end()) {
            soma += it->second * other_rating->second;
        }
    }
